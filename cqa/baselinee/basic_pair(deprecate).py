...
from prev.dataset import tf_data
from prev.utils import Object, ProgressBar, get_session, logger
...
class BasicPair:
    ...
    def init(self):
        self.tb_dirs = []
        self.l2_weights = []
        self.data = tf_data(self.dataset)

    def get_initializer(self):
        if self.init_f == 'normal':
            return tf.random_normal_initializer(stddev=self.init_v)
        return tf.random_uniform_initializer(minval=-self.init_v, maxval=self.init_v)

    def user_emb(self, u, name='UserEmb'):
        trainable = False
        if self.init_user == 'random':
        ...
        with tf.variable_scope(name):
            emb_w = tf.get_variable(
                name='emb_w',
                shape=(self.data.nb_users, self.dim_k),
                initializer=initializer,
                trainable=trainable,
            )
            if self.train_user == 'train':
                self.emb_ws.add(emb_w)
            elif self.train_user == 'dense':
                emb_w = tf.layers.dense(emb_w, self.dim_k, name='MapUserVec')
            if self.phase == 'pos':
    ...
    def text_rep(self, t, mask):
        if self.text_mode == 'top_k_sum':
            t = self.top_k_sum(t, self.top_k)
        elif self.text_mode == 'mask_top_k_sum':
            t = self.mask_top_k_sum(t, mask, self.top_k)
        elif self.text_mode == 'top_k_mean':
            t = self.top_k_mean(t, self.top_k)
        elif self.text_mode == 'mask_top_k_mean':
            t = self.mask_top_k_mean(t, mask, self.top_k)
        return t
    ...
    def TopLayer(self, o):
        ...
        with tf.variable_scope('TopLayer'):
            for i, n in enumerate(self.fc):
                ...
                if 0 < self.dp < 1:
                    o = tf.layers.dropout(o, rate=self.dp)
            # o = tf.layers.batch_normalization(o)
            o = tf.layers.dense(o, 1, name='dense_{}'.format(len(self.fc)))
            o = tf.reshape(o, (-1,))
        return o
    ...
    def MLP(self, x, units, activation=tf.nn.relu, name='MLP'):
        with tf.variable_scope(name):
            for i, n in enumerate(units):
                x = tf.layers.dense(x, n, activation=activation, name='dense_{}'.format(i))
        return x
        ...
        if self.emb_ws:
            if self.lr2 is None:
                self.minimize = optimizer.minimize(
                    self.loss, var_list=tf.trainable_variables() + list(self.emb_ws))
                self.minimize2 = None
            else:
                self.minimize = optimizer.minimize(self.loss)
                opt2 = opt(learning_rate=self.lr2)
    ...
    def pre_fit(self):
        self.data.set_args(
            ...
            wordVec_size=self.dim_k if self.init_word == 'word2vec' else 0,
        )
        # print('change size:', self.args.dim_k); input()
        if self.rank:
            data_generator = self.data.gen_train_pair(batch_size=self.batch_size)
        else:
            data_generator = self.data.gen_data(batch_size=self.batch_size)
        queue_size = 10
    ...
    def fit(self, args, args_i, args_n):
        ...
        self.dim_k = self.args.dim_k
        self.top_k = self.args.top_k
        self.init_f = self.args.init_f
        self.init_v = self.args.init_v
        self.init_user = self.args.init_user
        self.init_word = self.args.init_word
        self.train_word = self.args.train_word
        self.train_user = self.args.train_user
        self.text_mode = self.args.text_mode
        self.pair_mode = self.args.pair_mode
        self.ans_bias = self.args.ans_bias
        self.user_bias = self.args.user_bias
        self.batch_size = self.args.batch_size
        self.add_features = self.args.add_features
        self.rank = self.args.rank
        self.opt = self.args.opt
        self.lr = self.args.lr
        self.lr2 = self.args.lr2
        self.fc = self.args.fc
        ...
        try:
            for epochs in range(self.max_epochs):
                loss = []
                progress_bar = ProgressBar(self.args.batch_steps, msg='training')
