...
class V4(V1):
    ...
    def forward(self):
        u_emb = tf.nn.embedding_lookup(self.user_embed, self.uint_seq, name='user_lookup')
        q_emb = self.get_doc_embed(tf.expand_dims(self.qwid, axis=0), name='q_lookup_mask')
        a_emb = self.get_doc_embed(self.awid_seq, name='a_lookup_mask')
