...
class BertModel(object):
    ...
    Example usage:
    ```python
    # Already been converted into WordPiece token ids, two samples
    ...
    # label scores for each sample on each class of label
...
def get_activation(activation_string):
    ...
    act_name = activation_string.lower()
    lookup = {"linear": None, "relu": tf.nn.relu, "gelu": gelu, "tanh": tf.nn.tanh}
    return lookup[act_name]
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
    ...
    for x in init_vars:
        ...
        initialized_var_names.add(name)
        initialized_var_names.add(name + ":0")
        # initialized_var_names[name] = 1
        # initialized_var_names[name + ":0"] = 1
    return assignment_map, initialized_var_names
...
def embedding_lookup(input_ids,
                     ...
                     word_embedding_name="word_embeddings",
                     use_one_hot_embeddings=False):
    """Looks up words embeddings for input_ids tensor.
...
def embedding_postprocessor(input_tensor,
    ...
    if use_token_type:
        ...
        token_type_table = tf.get_variable(
            ...
            shape=[token_type_vocab_size, depth],
            initializer=create_initializer(initializer_range))
        ...
        token_type_embeddings = tf.reshape(token_type_embeddings, [batch_size, seq_length, depth])
    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            full_position_embeddings = tf.get_variable(
                ...
                shape=[max_position_embeddings, depth],
                initializer=create_initializer(initializer_range))
            ...
            #
            # So `full_position_embeddings` is effectively an embedding table for position
            # [0, 1, 2, ..., max_position_embeddings-1], and the current sequence has
            # positions [0, 1, 2, ... seq_length-1], so we can just perform a slice.
            position_embeddings = tf.slice(full_position_embeddings, [0, 0], [seq_length, -1])
            num_dims = len(output.shape.as_list())
            # Only the last two dimensions are relevant (`seq_length` and `depth`), so
            # we broadcast among the first dimensions, which is typically just
            # the batch size. (但是这里实际上用expand_dims就能达到一样的效果)
            position_broadcast_shape = [1] * (num_dims - 2) + [seq_length, depth]
...
def attention_layer(from_tensor,
    ...
    #   H = `size_per_head`
    from_tensor_2d = reshape_to_matrix(from_tensor)  # 这里只保留输入的最后一维，其他维度展平
    ...
    def transpose_for_scores(input_tensor, batch_size, num_attention_heads, seq_length, depth):
        out_shape = [batch_size, seq_length, num_attention_heads, depth]
        output_tensor = tf.reshape(input_tensor, out_shape)  # (B*T, N*H) -> (B, T, N, H)
        output_tensor = tf.transpose(output_tensor, [0, 2, 1, 3])  # (B, T, N, H) -> (B, N, T, H)
        return output_tensor
    ...
    if attention_mask is not None:
        ...
        # 因为`attention_scores`的第二维是head所在，所以这里扩充第二个维度的意思就是每个head
        # 都是使用同样的mask
        attention_mask = tf.expand_dims(attention_mask, axis=[1])
        # Since attention_mask is 1.0 for positions we want to attend and 0.0 for
        # masked positions, this operation will create a tensor which is 0.0 for
        # positions we want to attend and -10000.0 for masked positions.
        mask = (1.0 - tf.cast(attention_mask, tf.float32)) * -10000.0
        ...
        with tf.variable_scope("layer_%d" % layer_idx):
            ...
            with tf.variable_scope("attention"):
                ...
                if len(attention_heads) == 1:
                    ...
                    print('I don\'t think this code would ever be executed')
...
def reshape_to_matrix(input_tensor):
    ...
    output_tensor = tf.reshape(input_tensor, [-1, depth])  # 展平最后一维以外的其余维度
    return output_tensor
