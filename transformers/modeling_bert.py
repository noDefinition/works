...
def load_tf_weights_in_bert(model, config, tf_checkpoint_path):
    ...
    for name, shape in init_vars:
        logger.info("Loading TF weight {}, shape {}".format(name, shape))
    ...
    for name, array in zip(names, arrays):
        name = name.split('/')
        # adam_v and adam_m are variables in AdamWeightDecayOptimizer to calculated m and v
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        if attention_mask is not None:
            # Apply attention mask (precomputed for all layers in BertModel forward())
